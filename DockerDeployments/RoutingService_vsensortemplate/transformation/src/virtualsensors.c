...
struct DDS_DynamicData * VirtualSensorTransformationPlugin_createOutputSample(
    ...
    retCode = DDS_DynamicData_set_string(
        output, "unit", DDS_DYNAMIC_DATA_MEMBER_ID_UNSPECIFIED,"cpsds:Fahrenheit" );
    if (retCode != DDS_RETCODE_OK) {
        DDS_DynamicData_delete(output);
        return NULL;
    }

    DDS_Double CtoFahrenheit = (double)doubleValue * 1.8 + 32;

    if(self->value == -255){
	retCode = DDS_DynamicData_set_double(
        output, "value", DDS_DYNAMIC_DATA_MEMBER_ID_UNSPECIFIED, CtoFahrenheit);
	if(retCode != DDS_RETCODE_OK) {
            DDS_DynamicData_delete(output);
            return NULL;
	}
