...
/**
* A sequence of TemperatureSensor instances.
*/
public final class TemperatureSensorSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TemperatureSensorSeq() {
        super(TemperatureSensor.class);
    }

    public TemperatureSensorSeq (int initialMaximum) {
        super(TemperatureSensor.class, initialMaximum);
    }

    public TemperatureSensorSeq (Collection<?> elements) {
        super(TemperatureSensor.class, elements);
    }

    public  TemperatureSensor get(int index) { 
        return (TemperatureSensor) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TemperatureSensor.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TemperatureSensor objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TemperatureSensor.create());
